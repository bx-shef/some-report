...
describe('плейсменты', () => {
  ...
  // ⚠ Названия обязаны совпадать ПОИМЁННО, а не просто быть непустыми: из `PLACEMENTS` берётся
  // пункт меню портала, из `APP_REPORTS` — плитка на главной приложения и заголовок вкладки
  // (`reportTitle`). Разъедутся — человек нажмёт в меню одно название, а в отчёте прочитает
  // другое и решит, что открыл не то. Заголовок пункта к тому же применяется только при
  // `placement.bind`: на установленных порталах расхождение переживёт выкат.
  it('название пункта меню и название отчёта — одно и то же', () => {
    expect(PLACEMENTS.map(p => p.title)).toEqual(APP_REPORTS.map(report => report.title))
  })
