...
describe('права приложения', () => {
  ...
  // владельца. `telephony` добавлено 2026-09-06 для отчёта «Активность пользователей»: у дел CRM
  // нет ни длительности звонка, ни кода завершения, а отчёт считает только успешные разговоры
  // дольше порога.
  it('просит ровно четыре права и ни одного лишнего', () => {
    expect([...B24_REQUIRED_SCOPES].sort()).toEqual(['crm', 'placement', 'telephony', 'user_brief'])
  })
