import { describe, expect, it } from 'vitest'
import {
  PERIOD_PRESETS,
  fromIsoDate,
  matchPreset,
  periodLengthDays,
  resolvePreset,
  toIsoDate,
  validatePeriod, samePeriod } from '~/utils/period'

/** 3 сентября 2026 — тот самый день, когда заказчик открыл отчёт и увидел пустой месяц. */
const TODAY = new Date(2026, 8, 3)

describe('toIsoDate', () => {
  it('печатает локальную дату', () => {
    expect(toIsoDate(new Date(2026, 8, 1))).toBe('2026-09-01')
  })

  // ⚠ Через `toISOString()` восточнее Гринвича полночь 1 сентября превратилась бы в 31 августа:
  // отчёт захватывал бы лишние сутки в начале периода и терял их в конце.
  it('не уезжает на сутки от часового пояса', () => {
    expect(toIsoDate(new Date(2026, 0, 1))).toBe('2026-01-01')
    expect(toIsoDate(new Date(2026, 11, 31))).toBe('2026-12-31')
  })
})

describe('fromIsoDate', () => {
  it('разбирает ISO-дату', () => {
    expect(toIsoDate(fromIsoDate('2026-09-15')!)).toBe('2026-09-15')
  })

  // `new Date(2026, 1, 31)` молча становится 3 марта — такую «дату» в фильтр пускать нельзя.
  it('несуществующую дату не выдумывает', () => {
    expect(fromIsoDate('2026-02-31')).toBeUndefined()
    expect(fromIsoDate('2026-13-01')).toBeUndefined()
  })

  it('мусор отвергает', () => {
    expect(fromIsoDate('01.09.2026')).toBeUndefined()
    expect(fromIsoDate('')).toBeUndefined()
  })
})

describe('готовые интервалы', () => {
  it('текущий месяц — от первого до последнего числа', () => {
    expect(resolvePreset('this-month', TODAY)).toEqual({ from: '2026-09-01', to: '2026-09-30' })
  })

  // ⚠ Ради этого пресет и заведён: отчёт, открытый 3-го числа, показывает три дня — почти пустой
  // экран, который читается как поломка. Полный прошлый месяц и есть тот ответ, за которым пришли.
  it('прошлый месяц — полный', () => {
    expect(resolvePreset('prev-month', TODAY)).toEqual({ from: '2026-08-01', to: '2026-08-31' })
  })

  it('прошлый месяц в январе — это декабрь прошлого года', () => {
    expect(resolvePreset('prev-month', new Date(2026, 0, 10))).toEqual({ from: '2025-12-01', to: '2025-12-31' })
  })

  it('февраль високосного года считается верно', () => {
    expect(resolvePreset('this-month', new Date(2028, 1, 10))?.to).toBe('2028-02-29')
  })

  // 3 сентября 2026 — четверг: неделя с понедельника 31 августа по воскресенье 6 сентября.
  it('текущая неделя — с понедельника по воскресенье, через границу месяца', () => {
    expect(resolvePreset('this-week', new Date(2026, 8, 3))).toEqual({ from: '2026-08-31', to: '2026-09-06' })
  })

  it('в воскресенье текущая неделя заканчивается сегодня, в понедельник — начинается', () => {
    expect(resolvePreset('this-week', new Date(2026, 8, 6))).toEqual({ from: '2026-08-31', to: '2026-09-06' })
    expect(resolvePreset('this-week', new Date(2026, 8, 7))).toEqual({ from: '2026-09-07', to: '2026-09-13' })
  })

  it('прошлая неделя — семь дней перед текущей', () => {
    expect(resolvePreset('prev-week', new Date(2026, 8, 3))).toEqual({ from: '2026-08-24', to: '2026-08-30' })
  })

  // 29 декабря 2026 — вторник: неделя уходит в следующий год; 4 января 2026 — воскресенье
  // недели, начавшейся в прошлом году.
  it('неделя через границу года считается верно в обе стороны', () => {
    expect(resolvePreset('this-week', new Date(2026, 11, 29))).toEqual({ from: '2026-12-28', to: '2027-01-03' })
    expect(resolvePreset('this-week', new Date(2026, 0, 4))).toEqual({ from: '2025-12-29', to: '2026-01-04' })
    expect(resolvePreset('prev-week', new Date(2027, 0, 4))).toEqual({ from: '2026-12-28', to: '2027-01-03' })
  })

  it('неделя с 29 февраля високосного года', () => {
    expect(resolvePreset('this-week', new Date(2028, 1, 29))).toEqual({ from: '2028-02-28', to: '2028-03-05' })
  })

  // В воскресенье «последние 7 дней» совпадают с календарной неделей — подсветка называет календарь.
  it('в воскресенье совпавшие границы называются текущей неделей, а не последними 7 днями', () => {
    expect(matchPreset({ from: '2026-08-31', to: '2026-09-06' }, new Date(2026, 8, 6))).toBe('this-week')
  })

  it('последние 7 дней включают сегодня', () => {
    expect(resolvePreset('last7', TODAY)).toEqual({ from: '2026-08-28', to: '2026-09-03' })
  })

  it('последние 30 дней включают сегодня', () => {
    expect(periodLengthDays(resolvePreset('last30', TODAY)!)).toBe(30)
  })

  it('сегодня и вчера — по одному дню', () => {
    expect(resolvePreset('today', TODAY)).toEqual({ from: '2026-09-03', to: '2026-09-03' })
    expect(resolvePreset('yesterday', TODAY)).toEqual({ from: '2026-09-02', to: '2026-09-02' })
  })

  it('квартал считается от начала своей тройки месяцев', () => {
    expect(resolvePreset('this-quarter', TODAY)).toEqual({ from: '2026-07-01', to: '2026-09-30' })
    expect(resolvePreset('this-quarter', new Date(2026, 0, 5))).toEqual({ from: '2026-01-01', to: '2026-03-31' })
  })

  it('год — целиком', () => {
    expect(resolvePreset('this-year', TODAY)).toEqual({ from: '2026-01-01', to: '2026-12-31' })
  })

  it('произвольный интервал сам себя не вычисляет', () => {
    expect(resolvePreset('custom', TODAY)).toBeUndefined()
  })

  it('у каждого интервала есть подпись', () => {
    for (const preset of PERIOD_PRESETS) expect(preset.label.trim()).not.toBe('')
  })
})

describe('matchPreset', () => {
  // Иначе после ручного ввода «01.09 — 30.09» подсветится «Произвольный», и человек видит, что
  // система не понимает того, что он только что выбрал.
  it('узнаёт готовый интервал во введённых руками датах', () => {
    expect(matchPreset({ from: '2026-09-01', to: '2026-09-30' }, TODAY)).toBe('this-month')
    expect(matchPreset({ from: '2026-08-01', to: '2026-08-31' }, TODAY)).toBe('prev-month')
  })

  /**
   * В последний день 30-дневного месяца «текущий месяц» и «последние 30 дней» совпадают дословно.
   * Побеждает календарный — это правило `MATCH_ORDER`, а не случайность.
   *
   * ⚠ Вторая пара того же рода после перестановки кнопок: на экране «последние 30 дней» теперь
   * стоят РАНЬШЕ «текущего месяца», и разбор обязан по-прежнему выбирать месяц.
   */
  it('при совпадении границ выбирает календарный интервал, а не «последние N дней»', () => {
    const lastDayOfApril = new Date(2026, 3, 30)
    expect(matchPreset({ from: '2026-04-01', to: '2026-04-30' }, lastDayOfApril)).toBe('this-month')
    const ids = PERIOD_PRESETS.map(preset => preset.id)
    expect(ids.indexOf('last30')).toBeLessThan(ids.indexOf('this-month'))
  })

  it('чужие границы называет произвольными', () => {
    expect(matchPreset({ from: '2026-08-15', to: '2026-09-02' }, TODAY)).toBe('custom')
  })

  /**
   * ⚠ Порядок КНОПОК и порядок РАЗБОРА — разные вещи, и когда-то это была одна и та же строка.
   * Владелец 2026-09-06 попросил переставить кнопки: сегодня, вчера, 7 дней, неделя… То есть
   * «последние 7 дней» встали ПЕРЕД «текущей неделей», а в воскресенье их границы совпадают.
   * Будь разбор привязан к порядку кнопок, подсветка молча начала бы называть выбор чужим именем.
   * Этот тест держит их врозь: он падает, если разбор снова поедет за расстановкой.
   */
  it('перестановка кнопок не меняет того, как называется совпавший интервал', () => {
    const ids = PERIOD_PRESETS.map(preset => preset.id)
    expect(ids.indexOf('last7')).toBeLessThan(ids.indexOf('this-week'))
    expect(matchPreset({ from: '2026-08-31', to: '2026-09-06' }, new Date(2026, 8, 6))).toBe('this-week')
  })
})

describe('порядок кнопок периода', () => {
  /**
   * Расстановка владельца от 2026-09-06: от мелкого к крупному, «Произвольный» последним.
   *
   * ⚠ Тест на порядок нужен именно потому, что порядок здесь — единственное, что задаёт вид
   * панели: кнопки рисуются перебором этого массива. Поменяют местами «ради красоты» — экран
   * поедет молча, и заметит это только заказчик.
   */
  it('идут от дня к году, произвольный — последним', () => {
    expect(PERIOD_PRESETS.map(preset => preset.id)).toEqual([
      'today', 'yesterday', 'last7', 'last30',
      'this-week', 'prev-week', 'this-month', 'prev-month',
      'this-quarter', 'this-year', 'custom'
    ])
  })

  it('умолчание отчётов — «текущий месяц» — среди кнопок есть', () => {
    expect(PERIOD_PRESETS.some(preset => preset.id === 'this-month')).toBe(true)
  })

  /**
   * ⚠ Порядок разбора — отдельный список, и TypeScript его полноту не проверяет. Забудут внести
   * туда новый пресет — его границы перестанут узнаваться: кнопка не подсветится, а вместо неё
   * откроется календарь произвольных дат. Молча, потому что остальные тесты берут конкретные
   * даты и мимо нового пресета проходят.
   */
  it('в разборе учтён каждый пресет, кроме произвольного', () => {
    const withBounds = PERIOD_PRESETS.filter(preset => preset.resolve).map(preset => preset.id)
    for (const id of withBounds) {
      const bounds = resolvePreset(id, TODAY)!
      // Границы каждого пресета обязаны узнаваться — хотя бы им самим, хотя бы совпавшим соседом.
      expect(matchPreset(bounds, TODAY), `${id} не узнаётся разбором`).not.toBe('custom')
    }
    expect(PERIOD_PRESETS.filter(preset => !preset.resolve).map(preset => preset.id)).toEqual(['custom'])
  })
})

describe('periodLengthDays', () => {
  it('считает обе границы включительно', () => {
    expect(periodLengthDays({ from: '2026-09-01', to: '2026-09-01' })).toBe(1)
    expect(periodLengthDays({ from: '2026-09-01', to: '2026-09-30' })).toBe(30)
  })

  // ⚠ Прогон идёт в UTC, где перевода часов нет, — обычный вызов ничего бы не доказал: прошлая
  // формула на нём была зелёной и при этом недосчитывала день в Берлине. Поэтому пояс ставим явно.
  it('не сбивается на переводе часов (Europe/Berlin, 23-часовые сутки 29 марта)', () => {
    const previous = process.env.TZ
    process.env.TZ = 'Europe/Berlin'
    try {
      expect(periodLengthDays({ from: '2026-03-28', to: '2026-03-30' })).toBe(3)
      expect(periodLengthDays({ from: '2026-03-01', to: '2026-04-01' })).toBe(32)
    } finally {
      process.env.TZ = previous
    }
  })
})

describe('validatePeriod', () => {
  it('годный период не трогает', () => {
    expect(validatePeriod({ from: '2026-09-01', to: '2026-09-30' })).toBeUndefined()
  })

  // ⚠ Перевёрнутый период REST принимает без ошибки и возвращает пустой список — отчёт показал бы
  // нули, неотличимые от «за период ничего не было», и человек искал бы ошибку в CRM.
  it('перевёрнутый период ловит и предлагает починку', () => {
    const problem = validatePeriod({ from: '2026-09-30', to: '2026-09-01' })
    expect(problem?.message).toContain('поменяны местами')
    expect(problem?.fixed).toEqual({ from: '2026-09-01', to: '2026-09-30' })
  })

  it('слишком длинный период отклоняет', () => {
    expect(validatePeriod({ from: '2024-01-01', to: '2026-01-01' })?.message).toContain('покороче')
  })

  it('битые даты отклоняет до похода в портал', () => {
    expect(validatePeriod({ from: 'вчера', to: '2026-09-30' })?.message).toContain('неверно')
  })

  it('ровно граничная длина проходит', () => {
    expect(validatePeriod({ from: '2026-01-01', to: '2026-01-10' }, 10)).toBeUndefined()
    expect(validatePeriod({ from: '2026-01-01', to: '2026-01-11' }, 10)?.message).toContain('покороче')
  })
})

describe('samePeriod', () => {
  it('сравнивает по границам, пустое — не совпадает ни с чем', () => {
    expect(samePeriod({ from: '2026-08-01', to: '2026-08-31' }, { from: '2026-08-01', to: '2026-08-31' })).toBe(true)
    expect(samePeriod({ from: '2026-08-01', to: '2026-08-31' }, { from: '2026-08-01', to: '2026-08-30' })).toBe(false)
    expect(samePeriod(undefined, { from: '2026-08-01', to: '2026-08-31' })).toBe(false)
  })
})
