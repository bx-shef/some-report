...
describe('страница отчёта в портале', () => {
  ...
  it('под фильтром: пустой период — про фильтр, причина проигрыша и лиды периода — плашки, блок 7 подписан', async () => {
    const wrapper = await mountSuspended(AppPage)
    await vi.waitFor(() => expect(mainKeys()).toHaveLength(1))
    const from = mainKeys()[0]!
    portal.pending[from]!([])
    await vi.waitFor(() => expect(wrapper.text()).toContain('1. Сводка'))
    // Без фильтров края нет: сделки периода берутся из ЛЮБЫХ лидов, и оговорке неоткуда взяться.
    expect(wrapper.text()).not.toContain('Сделки — только из лидов этого периода')
    ...
    // Источник ложится на сделки СПИСКОМ ID ЛИДОВ, а лиды спрашиваются по периоду — значит, край
    // вопроса 17 здесь есть, и экран обязан о нём сказать (решение владельца 2026-09-18).
    expect(wrapper.text()).toContain('Сделки — только из лидов этого периода')
    ...
    // ⚠ А причина проигрыша ложится на сделки НАПРЯМУЮ, массивом кодов стадий: списка ID лидов
    // здесь нет, значит нет и края. Плашка «на всякий случай» под любым фильтром обесценила бы
    // себя — её перестали бы читать там, где она про дело.
    expect(wrapper.text()).not.toContain('Сделки — только из лидов этого периода')
  })

  /**
   * Оговорка про лиды периода — свойство ЖИВОЙ выборки: сделки под таким фильтром отбираются
   * списком ID лидов, а лиды спрашиваются по периоду их создания. В демо-наборе нет ни портала,
   * ни этой выборки, и плашка обещала бы придуманным числам свойство, которого у них нет.
   */
  it('вне портала оговорки про лиды периода нет даже под фильтром', async () => {
    ...
    demo.findComponent(ReportFilters).vm.$emit('update:modelValue', { sourceId: 'CALL' })
    // ⚠ Ждём именно ПРИМЕНЕНИЯ фильтра, а не перерисовки: «1. Сводка» на экране уже была, и
    // проверка отсутствия плашки прошла бы, не дождавшись фильтра вовсе — то есть ни о чём.
    // «Онлайн-чат» — чужой источник в блоке 5; под фильтром «Входящий звонок» строка уходит.
    await vi.waitFor(() => expect(demo.text()).not.toContain('Онлайн-чат'))
    expect(demo.text()).toContain('Входящий звонок')
    expect(demo.text()).not.toContain('Сделки — только из лидов этого периода')
    demo.unmount()
  })
