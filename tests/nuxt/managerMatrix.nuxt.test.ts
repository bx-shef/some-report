...
/**
 * Матрица «офис → менеджер → стадия». Проверяем то, что ломается молча: какие числа кликабельны,
...
describe('ManagerMatrix', () => {
  it('рисует карточку на офис и строку на менеджера', async () => {
  ...
  it('клик по клетке уходит списком ровно за это число', async () => {
    ...
    expect(cell).toMatchObject({ officeId: 10, managerId: 1, stageId: 'NEW', total: 5 })
    expect(cell.title).toBe('Сделки: Минск · Иванов Иван · Новая')
  })

  it('итог строки — список без стадии, итог офиса — без менеджера', async () => {
    const wrapper = await mount()
    ...
    expect(wrapper.emitted('drill')?.[0]?.[0]).toMatchObject({ officeId: 10, managerId: 1, total: 12 })
    expect((wrapper.emitted('drill')?.[0]?.[0] as ManagerCellRef).stageId).toBeUndefined()

    // Именно ПЕРВОЙ карточки: `tfoot` есть у каждого офиса, и последняя — «не указана».
    const officeTotal = wrapper.findAll('table')[0]!.findAll('tfoot button').at(-1)
    await officeTotal!.trigger('click')
    const office = wrapper.emitted('drill')?.[1]?.[0] as ManagerCellRef
    expect(office).toMatchObject({ officeId: 10, total: 25 })
    expect(office.managerId).toBeUndefined()
  })
  ...
  it('сделки офиса вне строк показаны отдельной строкой с объяснением', async () => {
    const text = (await mount()).text()
    expect(text).toContain('Ответственный не указан или не найден')
    expect(text).toContain('разница между итогом офиса и суммой строк')
  })
})
