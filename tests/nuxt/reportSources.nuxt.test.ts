...
describe('ReportSources', () => {
  ...
  /**
   * ⚠ Итог таблицы считается по её собственным строкам, а не берётся из сводки: сделки, чьего
   * источника лида отчёт не знает, в разрез не входят. Отчёт обязан объяснить расхождение сам.
   */
  it('объясняет выручку, которой нет в таблице', async () => {
    ...
    expect(wrapper.text()).toContain('вне разреза')
    // ⚠ Подпись называет ОБЕ причины: своего лида у сделки может не быть вовсе, а может быть
    // лид прошлого периода — источника такой сделки в таблице тоже нет.
    expect(wrapper.text()).toContain('создан раньше периода')
