...
describe('ActivityToolbar: период ограничен месяцем', () => {
  /**
   * ⛔ Решение владельца 2026-09-06. Длинные интервалы НЕ показываются вовсе, а не «показать и
   * отругать»: кнопка, которая всегда отвечает жалобой, — приглашение к ошибке.
   *
   * ⚠ Дело не в ожидании (месяц пакетами читается за секунды), а в том, что звонки лежат СТРОКАМИ
   * в памяти фрейма: квартал — под шестьдесят тысяч записей, и пересчитываются они на каждую
   * смену порога.
   */
  it('квартала и года среди интервалов нет', async () => {
    const wrapper = await mount()
    const text = wrapper.text()
    expect(text).toContain('Текущий месяц')
    expect(text).toContain('Прошлый месяц')
    expect(text).toContain('Последние 30 дней')
    expect(text).not.toContain('Текущий квартал')
    expect(text).not.toContain('Текущий год')
  })

  /** ⚠ Умолчание отчёта — текущий месяц: оно обязано влезать в собственный предел. */
  it('умолчание отчёта проходит собственный предел', async () => {
    const wrapper = await mount()
    expect(wrapper.text()).toContain('Текущий месяц')
    // Жалобы на экране быть не должно: предел 31 день, а в месяце их не больше.
    expect(wrapper.text()).not.toContain('выберите промежуток покороче')
  })
})
