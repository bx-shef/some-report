...
describe('выбор периода', () => {
  ...
  it('«Прошлый месяц» просит август целиком', async () => {
    const wrapper = await mount({ period: { from: '2026-09-01', to: '2026-09-30' } })
    await pick(wrapper, 'Прошлый месяц')
    expect(wrapper.emitted('update:period')).toEqual([[{ from: '2026-08-01', to: '2026-08-31' }]])
  })

  /**
   * ⚠ Нажатие на уже подсвеченную кнопку в портал НЕ уходит. Панель открыта на августе, «Прошлый
   * месяц» — это он и есть; стоило бы такое нажатие полной выборки отчёта: на боевом портале это
   * десятки секунд запросов и лишняя запись отбора в настройки портала.
   */
  it('повторный выбор того же интервала ничего не просит', async () => {
    const wrapper = await mount()
    await pick(wrapper, 'Прошлый месяц')
