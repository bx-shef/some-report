...
export const PAGES: readonly PageCheck[] = [
  ...
  // Детализация: её открывает настоящий слайдер портала. Прямой заход честно говорит, что
  // показывать нечего, — и это тоже рисует клиент, а не разметка.
  { path: '/app/drill/', mustContain: ['Нечего показывать'] },
