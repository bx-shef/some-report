...
/**
 * Фильтр по менеджеру, источнику или стадии лида ложится на сделки СПИСКОМ ID ЛИДОВ, а лиды
 * спрашиваются по дате создания. Значит, сделка выбранного периода, заведённая из лида
 * предыдущего, под таким фильтром не посчитана — хотя без фильтров она в отчёт входит.
 *
 * ⚠ Край принят владельцем (решение 2026-09-18, вопрос 17 в `docs/METRICS.md`): считать иначе —
 * спрашивать ID лидов без ограничения периода, а это сотни тысяч записей. Раз край остаётся, о
 * нём обязан знать тот, кто смотрит на числа, а не только тот, кто читает документацию: молча
 * заниженная строка «Успешные сделки из лидов» неотличима от плохого месяца.
 *
 * ⚠ Условие — та же `needsLeadIds`, что решает, идти ли за списком ID лидов. Не «менеджер или
 * стадия» перечислением: источник ложится на сделки тем же списком (`SOURCE_ID` у сделки есть,
 * но спрашивать его напрямую нельзя — см. `filters.ts`), и забытый здесь источник оставил бы
 * ровно тот же край без единого слова на экране.
 *
 * ⚠ В демо-наборе края нет вовсе: там ни портала, ни выборки лидов по периоду, — и плашка над
 * придуманными числами обещала бы им свойство, которого у них нет.
 */
const leadPeriodNote = computed(() =>
  needsLeadIds(appliedFilters.value) && !pending.value && !isDemo.value
    ? 'Под этим фильтром сделки отбираются через их лиды, а лиды — по дате создания. Поэтому сделка выбранного периода, заведённая из лида предыдущего, здесь не посчитана: без фильтров она в отчёте есть. Лидов и квалификации это не касается — они и так считаются по дате создания лида.'
    : undefined
)
      ...
      <template v-else>
        ...
        >
          ...
          <template v-if="needsLeadIds(filters)">
            Под фильтром по менеджеру, источнику или стадии дольше: сначала лиды под фильтром, потом их сделки.
          </template>
        </p>
        ...
        <B24Alert
          v-if="leadPeriodNote"
          color="air-primary-warning"
          title="Сделки — только из лидов этого периода"
          :description="leadPeriodNote"
        />
