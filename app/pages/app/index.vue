 ...
 *
 * ⚠ Во фрейме слайдера экраны этой страницы занимают ВЕСЬ экран, а высоту фрейма мы не трогаем
 * вовсе. Подгонка под содержимое (`fitWindow`) делала из настоящего слайдера модальное окошко:
 * короткий экран — «Загрузка…» или плашка отказа — сжимал его до двух строк.
 */
...
onMounted(async () => {
  ...
  // ⚠ Фрейм детализации высоту не подгоняет НИКОГДА — в том числе когда условие оказалось
  // негодным. Плашка «Список не открылся» занимает две строки, и подгонка сжала бы слайдер
  // до них: человек, нажавший на число, увидел бы вместо объяснения полоску.
  if (!requested) await fit()
})

/**
 * Подогнать высоту фрейма под содержимое — ТОЛЬКО для оглавления приложения.
 *
 * ⛔ Детализации это ВРЕДИТ, и урок стоил трёх заходов. Портал открывает фрейм слайдера высотой
 * в экран; подгонка под содержимое схлопывала его до размера списка, и настоящий слайдер
 * выглядел модальным окошком посреди экрана. На боевом это читалось однозначно: «открылся не
 * тот слайдер» — девять сделок давали окно на треть экрана, а сотня лидов выглядела нормально.
 * Поэтому список живёт во весь экран и листается ВНУТРИ себя (`ReportDrill.vue`).
 *
 * ⚠ Оглавлению подгонка по-прежнему нужна: там содержимого на три плитки, и фрейм в целый экран
 * оставил бы под ними пустое поле. То же делают все остальные страницы приложения.
 */
...
/** Следующая страница списка. Высоту фрейма не трогаем — список листается внутри себя. */
...
<template>
  ...
  <main
    ...
    class="mx-auto h-screen max-w-4xl p-3 sm:p-4 lg:p-6"
  >
    <B24Alert
