...
export interface ReportMetrics {
  ...
  /**
   * «Итого» таблицы источников — по её СТРОКАМ, а не из сводки: разрез источников не берёт
   * сделки без лида-родителя (их источник неизвестен), сводка берёт все. Считается здесь, а не в
   * компоненте, чтобы экран и экспорт печатали одно число.
   */
  sourceTotals: SourceTotals
  /** Успешные сделки и выручка, которых нет в разрезе источников, — та самая разница со сводкой. */
  outsideSources: { deals: number, revenue: number }
}

/** Итог таблицы источников: те же колонки, что у строки источника, без кода. */
export type SourceTotals = Omit<SourceRow, 'sourceId'>
...
export interface ReportDataset {
  ...
  /**
   * ID лидов под фильтром по менеджеру или стадии — тот список, по которому выбраны сделки
   * (`LEAD_ID in`). Нужен детализации по клику: список сделок «тем же фильтром» строится по
   * нему, а не заново. Только с портала и только под таким фильтром.
   */
