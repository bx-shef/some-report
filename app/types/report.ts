...
export interface LeadAggregate {
  ...
  /**
   * Источник каждого лида по его идентификатору: по нему сделка находит источник СВОЕГО лида.
   *
   * ⛔ Поле ОБЯЗАТЕЛЬНОЕ, и это не формальность. У сделки есть собственный `SOURCE_ID`, поэтому
   * запасная ветка «нет карты — возьмём у сделки» выглядела безобидно и молча становилась
   * боевой: замер портала заказчика 2026-09-09 — вся выручка блока 5 (14 861,98 BYN на 20
   * успешных сделках) уходила в «Другие источники», потому что «Заказ покупателя» из 1С
   * приходит БЕЗ источника и привязывается к лиду руками. Обязательность значит, что агрегат
   * нельзя собрать, не ответив, откуда взялись источники: `adaptLeadCounts` отдаёт счётчики
   * БЕЗ карты (тип `LeadCounts`), а приставляет её тот, кто её прочитал, — и забыть этот шаг
   * теперь нельзя, компилятор не даст.
   */
...
/**
 * Счётчики лидов БЕЗ карты источников — то, что умеет отдать разбор ответов портала
 * (`adaptLeadCounts`).
 *
 * ⛔ Отдельный тип нужен ровно затем, чтобы карту нельзя было забыть: счётчики знают только
 * числа, лиды поимённо им неизвестны, а `sourceRows` без карты положила бы выручку на источник
 * САМОЙ сделки — он бывает пуст. Пока это был необязательный ключ одного типа, пропажа шага
 * «прочитать источники лидов» ничего не ломала: тесты чистых функций оставались зелёными
 * (проверено саботажем), а на боевом портале вся выручка уезжала в «Другие источники».
 * Теперь тот же саботаж — ошибка компиляции.
 */
export type LeadCounts = Omit<LeadAggregate, 'leadSourceById'>
...
export interface UnlinkedDeals {
  ...
  unconverted: number
  /**
   * Сделок в валюте, ОТЛИЧНОЙ от базовой: суммы приведены курсом.
   *
   * ⚠ Не то же, что `unconverted`: там валюта неизвестна (сломано), здесь курс нашёлся и всё
   * сработало верно. Показываем потому, что у заказчика все сделки должны быть в BYN — значит
   * сделка в другой валюте это ошибка ВВОДА, а приведённая курсом она неотличима в выручке от
   * правильной.
   */
  foreign: number
...
export interface SourceRow {
  ...
  revenue: number
  /**
   * Идентификаторы успешных сделок, вошедших в `won` и `revenue`, — для списка по клику.
   *
   * ⚠ Список за этим числом нельзя собрать условием REST: сделки отобраны по источнику ЛИДА, а
   * у `crm.deal.list` поля «источник моего лида» нет. Спросить сделку её собственным `SOURCE_ID`
   * значило бы показать ДРУГОЕ множество — то самое, из-за которого выручка и уходила не в свои
   * строки. Поэтому список открывается ровно этими записями (`ID in (...)`): не «тем же
   * условием», а буквально теми же сделками, что посчитаны.
   */
  wonDealIds: number[]
}
 ...
 *
 * ⚠ Списка сделок у итога тоже нет: строка «Итого» не кликается — за ней стоял бы весь разрез,
 * а такое число уже есть в сводке отчёта, вместе со сделками без лида.
 */
export type SourceTotals = Omit<SourceRow, 'sourceId' | 'wonDealIds'>
...
export interface ReportDictionaries {
  ...
  /**
   * Стадии сделок выбранного направления (код → имя) — чтобы список по клику печатал «Выставлен
   * счёт», а не `1`. Есть только у отчёта «Сделки по менеджерам»: у отчёта по лидам направление
   * не выбирается, а одинаковые коды в разных направлениях значат разное.
   */
