...
export function reportSheets(report: ReportMetrics, dataset: ReportDataset, filters: ReportFilters, isDemo: boolean, state: ExportState = {}): Sheet[] {
  ...
  if (unlinked) {
    ...
    // ⚠ Те же две оговорки и теми же словами, что на экране: лист Excel обязан сходиться с ним
    // построчно, иначе распечатанная выручка перестаёт объяснять сама себя ровно там, где её
    // сверяют с CRM. Разошлись они однажды именно здесь — экран переписали, а лист забыли.
    if (unlinked.unconverted > 0) rows.push(['Сделок в валюте, которой нет в справочнике портала — суммы сложены как есть', unlinked.unconverted, '', '', ''])
