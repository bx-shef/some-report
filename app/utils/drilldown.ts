...
export interface DrillListParams {
   ...
   *
   * ⚠ Признак говорит, КАК читать, и только. ЧЕМ показывать, решает `localPanelReason`: список
   * ID лидов, влезающий в один кусок, — обычное условие, и его принимает слайдер. Панель нужна
   * лишь когда кусков больше одного. Прежде эти два вопроса были склеены, и любой фильтр по полю
   * лида уводил сделки в панель — соседние числа одного экрана открывались разными окнами.
   */
  byLeadIds: boolean
  /**
...
export function drillListParams(request: DrillRequest, period: ReportPeriod, filters: ReportFilters, codesByReason: Record<string, string[]>): DrillListParams {
  ...
  // ничего не изменит. Разница не косметическая: с `byLeadIds` список читается кусками, и когда
  // их больше одного, условие в слайдер уже не помещается (`localPanelReason`) — под фильтром с
  // длинным списком лидов такое число открывалось бы панелью вместо слайдера.
...
/**
 * Почему список показывает ПАНЕЛЬ внутри отчёта, а не настоящий слайдер портала, — когда это
 * решаем МЫ, а не портал. `undefined` — условие самодостаточно, слайдеру его можно отдать.
 *
 * ⛔ Правило одно на все ветки: в слайдер уезжает условие, которое ПОМЕЩАЕТСЯ В ОДИН ЗАПРОС.
 * Раньше здесь стояло грубее — «фильтр по полю лида ⇒ панель», и это разводило соседние числа
 * одного экрана: «Квалифицировано в сделку» (лиды) открывалось слайдером, а «Успешные сделки из
 * лидов» — панелью, потому что сделки под таким фильтром отбираются списком ID лидов. Человек
 * видел два разных окна на двух соседних плитках и читал это как поломку.
 *
 * А список ID лидов помещается в запрос, пока он влезает в один кусок (`DEFAULT_ID_CHUNK`, 500
 * — проверено на боевом портале). Тогда `LEAD_ID: [...]` — такое же обычное условие, как
 * `STATUS_ID`, и слайдер его принимает.
 *
 * ⚠ Ноль кусков — это НЕ «ноль записей в условии»: пустой `LEAD_ID: []` портал не видит вовсе и
 * отдал бы весь период. Поэтому такой список остаётся панелью, которая честно говорит «пусто».
 *
 * @param chunkCount сколько кусков по 500 занимает список ID лидов (0 — лидов под фильтром нет)
 */
export function localPanelReason(params: DrillListParams, chunkCount: number): string | undefined {
  if (params.empty) return 'условие этого числа спорит с выбранным фильтром — записей под ними нет'
  if (!params.byLeadIds) return undefined
  if (chunkCount === 0) return 'под выбранным фильтром лидов нет — значит нет и сделок по ним'
  if (chunkCount > 1) return `лидов под фильтром больше ${DEFAULT_ID_CHUNK} — их список читается кусками и целиком в слайдер не помещается`
  return undefined
}
