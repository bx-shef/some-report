 ...
 */
/** Все виды тире и минуса: `‐` `‑` `‒` (дефисы), `–` (короткое тире), `—` (длинное тире), `−` (минус). */
const DASHES = '‐‑‒–—−'
...
export function normalizeReasonName(name: string): string {
  return name
    ...
    .toLowerCase()
    .replace(/ё/g, 'е')
    // Все виды тире и минуса → дефис.
    .replace(new RegExp(`[${DASHES}]`, 'g'), '-')
    .replace(/\s+/g, ' ')
    // Пробелы вокруг дефиса и косой черты не значат ничего: «Отказ -Нет» и «Отказ - Нет»,
    // «Тендер/биржа» и «Тендер / биржа» — одно.
    .replace(/\s*-\s*/g, '-')
    .replace(/\s*\/\s*/g, '/')
    // Точка в конце — привычка, а не смысл.
    .replace(/\.$/, '')
}

/*
 * ⚠ Известные слепые пятна — НАМЕРЕННО не сводятся, пока не встретятся на живых данных:
 * разные кавычки («ВИП» / "ВИП"), латинская «C» вместо кириллической «С» (без таблицы
 * похожих символов это задача без края), неразрывный пробел и невидимые символы внутри слова.
 * Цена ошибки в них — лишняя строка в таблице, а не склеенные разные причины.
 */
 ...
 * Защита односторонняя: код стадии, который сам начинается с `reason:`, пересёкся бы с
 * каноничным ключом — но такие коды Битрикс24 не генерирует (`NEW`, `C1:5`, `UC_XXXXX`).
 */
...
function betterWritten(current: string, candidate: string): string {
  // ⚠ Тире в пробелах — тоже аккуратно. Проверка по одному дефису пропускала живую пару
  // «Отказ -Нет нужного количества» / «Отказ – Нет нужного количества»: второе написание
  // ни разу не побеждало, и печаталось первое — ровно то, что читается как наша опечатка.
  const tidy = (name: string) => new RegExp(`\\s[${DASHES}-]\\s`).test(name)
