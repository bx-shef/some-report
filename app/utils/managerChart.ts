...
/**
 * Подпись сектора сделок, не попавших ни в одну строку.
 *
 * ⚠ Не «Без ответственного»: ответственный у сделки обязателен (замер 2026-09-05 — ни одной из
 * 689 523 сделок портала без него). Причина у этого сектора ровно одна — сотрудник не нашёлся
 * среди перечисленных, — и подпись обязана называть её, а не ту, которой не бывает. Коротко,
 * потому что это подпись В секторе кольца.
 */
export const CHART_UNLISTED_LABEL = 'Вне перечисления'
