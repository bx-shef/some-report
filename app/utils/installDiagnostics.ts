 ...
 *    и НЕ ПОКАЗЫВАЕТ виджеты (это прямо написано в документации точки встройки);
...
import { PLACEMENTS } from '~/config/b24'
 ...
 * (`CRM_ANALYTICS_MENU`) теперь НЕСКОЛЬКО наших пунктов — по одному на отчёт, — и проверка «код
 * встречается в списке» показывала бы зелёное «всё зарегистрировано», когда привязан только
 * один из них: человек открыл бы аналитику и не нашёл там нужный отчёт.
 */
...
export function installVerdict(state: InstallState): InstallVerdict {
  ...
  if (extras.length > 0) {
    return {
      ...
      // ⚠ Число наших отчётов берётся из `PLACEMENTS`, а не вписано словом. Здесь уже стояло
      // «двух», и после третьего отчёта фраза врала бы администратору ровно на том экране,
      // который создан, чтобы ему не врать.
      hint: `Кроме наших отчётов (${PLACEMENTS.length}) портал помнит ещё ${extras.length}: ${extras.map(row => row.handler || row.code).join(', ')}. Так бывает после обновления с прежней версии, где пункт был один. Нажмите «Перепривязать точки» — лишние снимутся.`
    }
  }

  return {
    level: 'ok',
    title: `Всё зарегистрировано — отчёты доступны в портале (${PLACEMENTS.length})`,
    hint: `Пункты ${PLACEMENTS.map(placement => `«${placement.title}»`).join(', ')} — раздел «CRM-аналитика», в левом меню раскройте «Приложения» (рядом с «Маркетплейс»). Меню кэшируется: если пунктов не видно, перезагрузите страницу портала целиком.`
  }
}
