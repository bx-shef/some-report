...
/**
 * Сколько значений влезает в одно условие-перечисление (`STAGE_ID`, `ID`).
 *
 * ⚠ Экспортируется потому, что потолок держат ОБЕ стороны: здесь — приёмник, на случай
 * подделанной нагрузки, а `drilldown.ts` сверяется с ним ЗАРАНЕЕ и делает число некликабельным.
 * Иначе список успешных сделок источника молча приехал бы обрезанным — короче числа над ним.
 */
