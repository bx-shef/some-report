...
/**
 * Сколько значений принимается в одном условии-перечислении (`STAGE_ID`, `ID`) от ПОРТАЛА.
 *
 * ⚠ Это граница НЕДОВЕРЕННОГО ввода, а не «сколько значений выдерживает фильтр портала». Второе
 * — `DEFAULT_ID_CHUNK` (500, проверено на боевом), и отправитель обязан сверяться именно с ним:
 * список длиннее одного запроса приехал бы обрезанным, то есть короче числа над ним. Здешний
 * потолок вдвое шире намеренно — он про размер сообщения и стоит с запасом к нашему условию.
 */
