 ...
 * ([`docs/PORTAL.md`](../../docs/PORTAL.md)). Цена — 381 страница на месяц, поэтому звонки читаются
...
export function buildActivityReport(input: ActivityInput): ActivityReport {
  ...
  // которых не отдал `user.get` (бот, внештатник, удалённая учётка). Спрятать их значило бы, что
  // сумма столбца «разговоры» не сходится с итогом; но и напечатать им ноль писем нельзя —
  // счётчиков дел им никто не задавал, и такой ноль читался бы как «не писал» вместо «не
  // спрашивали». Отсюда `deedsKnown: false`.
  //
  // ⚠ Сотрудников ЧУЖОГО ОТДЕЛА здесь быть не может: под выбранным отделом их звонки отсеивает
  // `scopeCallsToUsers` ещё до сборки. Строка без спрошенных дел появляется только когда отдел НЕ
  // выбран, — и экран обязан объяснять именно это.
