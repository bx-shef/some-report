...
export function useDrilldown(input: { dataset: Ref<ReportDataset>, filters: Ref<ReportFilters> }) {
   ...
   *
   * 1. условие не помещается в ОДИН запрос (`localPanelReason`): список ID лидов под фильтром
   *    длиннее одного куска — отправь мы туда первый кусок, список вышел бы УЖЕ числа, по
   *    которому нажали. ⚠ Пока список влезает в кусок, он уезжает в слайдер как обычное
   *    условие: панель здесь НЕ штатный путь для любого фильтра по полю лида;
