 ...
 * внутри отчёта. Поэтому `openDrill` не бросает, а ОТВЕЧАЕТ отказом с причиной
 * (`SliderOpenResult`): причину видит человек, которому вместо слайдера подняли панель.
 */
/**
...
export function usePortalSlider() {
  ...
  async function openDrill(payload: DrillSliderPayload, stillWanted: () => boolean = () => true): Promise<SliderOpenResult> {
    ...
    try {
      ...
      // ⚠ Текст исключения SDK на экран НЕ выносим — то же решение, что у `readDrill` ниже: он не
      // под нашим контролем, меняется с версией библиотеки и может унести в чужое окно что угодно.
      // Человеку нужен не стектрейс, а «что делать», и место отказа названо однозначно.
      return { opened: false, reason: 'портал отказал в открытии слайдера' }
    }
  }
