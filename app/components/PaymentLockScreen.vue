...
/**
 * Экран приостановки приложения: приём выполненных работ не оформлен.
 *
    ...
    <B24Card class="w-full max-w-3xl">
      <div class="flex flex-col items-center gap-6 md:flex-row md:items-center md:gap-8">
        ...
        <div class="min-w-0 flex-1">
          <!--
            ⚠ Формулировки НЕЙТРАЛЬНЫЕ, и это требование владельца от 2026-09-16, а не стилистика.
            Экран видит любой сотрудник, открывший отчёт, — рядовой менеджер, а не тот, кто ведёт
            расчёты. Текст вроде «работа не оплачена» выставлял бы его работодателя должником перед
            собственным коллективом: приложение не имеет права вмешиваться в отношения внутри
            компании клиента. Поэтому здесь говорится только о ФОРМАЛЬНОСТИ между организациями —
            приём работ не завершён, — и ни слова о деньгах, долге и чьей-либо вине.
          -->
          <B24Alert
            :color="soft ? 'air-primary-warning' : 'air-primary-alert'"
            :title="soft ? 'Приём работ по приложению не завершён' : 'Доступ к отчётам приостановлен'"
            :description="soft
              ? 'Отчёты работают в обычном режиме. Приложение ждёт, когда будет оформлен приём выполненных работ — это формальность между вашей организацией и исполнителем.'
              : 'Приём выполненных работ по приложению пока не оформлен. Отчёты откроются сразу после того, как это будет улажено.'"
          />

          <!--
            ⚠ Срок обязан быть назван. Просьба без даты читается как вежливая формальность, а
            «с 21.09 доступ будет приостановлен» — как срок, у которого есть последствие. Нет даты
            на сервере — нет и строки: выдуманный срок хуже отсутствующего.
          -->
          <p
            v-if="soft && hardFrom"
          ...
          >
            С {{ formatDate(hardFrom) }} доступ к отчётам будет приостановлен.
          </p>
          ...
          <template v-else>
            ...
            <p class="mt-2 text-sm opacity-70">
              Вопрос решается между вашей организацией и исполнителем работ — передайте, пожалуйста,
              это сообщение ответственному за приложение.
            </p>
          </template>
        </div>
      </div>
