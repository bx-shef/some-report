 ...
 * `sourceRows()` сознательно не берёт сделки, источник ЛИДА которых неизвестен (лида нет вовсе
 * или он создан раньше периода), а `summary` считает все. На живом портале, где сделки заводят
 * руками, итог не сходился со своей же колонкой — и это читалось как ошибка отчёта, хотя оба
 * числа верны.
 */
 ...
 * А успешные сделки и выручка сойтись НЕ обязаны — разрез источников не берёт сделки, чей
 * источник лида неизвестен, а сводка берёт все. Именно поэтому под таблицей стоит
 * строка-объяснение: без неё расхождение читалось бы как ошибка отчёта.
 */

/**
 * Сделки, которых нет в разрезе источников: у них неизвестен источник ЛИДА.
 *
 * ⚠ Причин ДВЕ, и называть только первую нельзя: лида нет вовсе (такие сделки читает блок 7) или
 * лид создан раньше периода и в карту источников не попал — на боевом портале это 37 сделок из
 * 947 за август. Подпись под таблицей говорит обе.
 */
