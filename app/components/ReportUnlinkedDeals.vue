...
<template>
  <B24Card>
    ...
    <template v-else-if="unlinked">
      ...
      <!--
        ⚠ Обе оговорки ЗОВУТ проверить, а не просто сообщают число, и обе — здесь, потому что в
        этом блоке 90 % сделок портала, то есть основная выручка.

        ⚠ Плашки называют РАЗНОЕ, и это главное. Замер боевого портала 2026-09-07: 403 сделки не
        в BYN (395 RUB, 7 USD, 1 EUR), курса ни у одной из этих валют в портале НЕТ — значит
        срабатывает именно ПЕРВАЯ оговорка, а вторая на этом портале покажет ноль. Ставить призыв
        к действию только на вторую значило бы не поставить его вовсе.

        ⚠ Плашка говорит «сумма не пересчитана» (факт), а не «выручка искажена» (вывод): у сделки
        в чужой валюте сумма бывает нулевой, и тогда выручка верна, а красная плашка над верной
        цифрой учит не верить оговоркам вообще.
      -->
      <p
      ...
      >
        <B24Badge
          ...
          size="xs"
        >
          Сумма не пересчитана
        </B24Badge>
        <span class="ml-2 opacity-80">
          Сделок в валюте, которой нет в справочнике портала: {{ formatCount(unlinked.unconverted) }} — суммы сложены с остальными КАК ЕСТЬ. Проверьте их в CRM: на эту разницу расходится выручка блока.
        </span>
      </p>

      <!--
        ⚠ Отдельной строкой: у заказчика ВСЕ сделки должны быть в базовой валюте («Все сделки в
        BYN. Судя по всему сделки в рублях это неправильные сделки», 2026-09-03). Значит это не
        «сделка в другой валюте», а ошибка ВВОДА — а приведённая курсом она неотличима в выручке
        от правильной, и найти её потом нельзя. Сумма при этом посчитана ВЕРНО, поэтому плашка
        другая и мягче.
      -->
      <p
        v-if="unlinked.foreign > 0"
        class="mb-3 text-xs"
      >
        <B24Badge
          color="air-primary-warning"
        ...
        <span class="ml-2 opacity-80">
          Сделок не в {{ currencyId }}: {{ formatCount(unlinked.foreign) }} — суммы приведены курсом, посчитаны верно. Проверьте их в CRM: по договорённости все сделки заводятся в одной валюте.
        </span>
      </p>
