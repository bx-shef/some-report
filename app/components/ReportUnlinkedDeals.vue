...
<template>
  <B24Card>
    ...
    <template v-else-if="unlinked">
      ...
      <!--
        ⚠ Зовём проверить, а не просто сообщаем число. Замер боевого портала 2026-09-07: 403
        сделки не в BYN (395 RUB, 7 USD, 1 EUR), курса ни у одной из этих валют в портале НЕТ, и
        162 успешных из них дают 22 211 807 «условных» — рубли, доллары и евро, сложенные с
        белорусскими как одно число. По самой цифре выручки это не видно никак.
      -->
      <p
      ...
      >
        ...
        <span class="ml-2 opacity-80">
          Сделок в валюте, которой нет в справочнике портала: {{ formatCount(unlinked.unconverted) }} — суммы вошли КАК ЕСТЬ, без пересчёта.
        </span>
      </p>

      <!--
        ⚠ Отдельной строкой и заметнее прочих оговорок: у заказчика ВСЕ сделки должны быть в
        базовой валюте («Все сделки в BYN. Судя по всему сделки в рублях это неправильные
        сделки», 2026-09-03). Значит это не «сделка в другой валюте», а ошибка ввода — а
        приведённая курсом она неотличима в выручке от правильной, и найти её потом нельзя.
      -->
      <p
      ...
      >
        ...
        <span class="ml-2 opacity-80">
          Сделок не в {{ currencyId }}: {{ formatCount(unlinked.foreign) }} — суммы приведены курсом и вошли в выручку.
        </span>
      </p>
